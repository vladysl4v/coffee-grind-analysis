...
```
6. *(Optional)* Generate precomputed augmented training data (~3× dataset size by default):
```bash
...
```
7. *(Required for hybrid models)* Precompute numerical features. Output CSVs are gitignored — each team member must run this locally:
```bash
