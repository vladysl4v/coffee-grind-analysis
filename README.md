...
uv run python src/split_labels.py
```
6. *(Optional)* Generate augmented training data (~3× dataset size by default):
```bash
# augment only (no segmentation on augmented images)
uv run python src/precompute_augmented_segmentation.py

# augment + apply segmentation to each augmented image
uv run python src/precompute_augmented_segmentation.py --segment

# custom multiplier
uv run python src/precompute_augmented_segmentation.py --augmentations-per-image 5 --segment
```
...
|---|---|---|
| `--model` | required | `resnet18`, `resnet152`, `resnext50`, `efficientnet_b0`, `vit`, `vit_large`, `simple_cnn` |
| `--epochs` | `50` | Number of training epochs |
| `--lr` | `1e-3` | Learning rate |
| `--batch-size` | `32` | Batch size |
| `--unfreeze` | off | Train with backbone fully unfrozen from epoch 1 |
| `--unfreeze-after N` | off | Freeze backbone for N epochs, then unfreeze at lr/10 |
| `--augmented-data` | off | Use `augmented_segmentation/` and `augmented_train.csv` (run step 6 first) |

Artifacts (loss curves, scatter plots, checkpoints, `config.json`) are saved to `data/runs/<model>/run_NNN/`.
