"""
Train Swin Transformer for coffee grind fineness regression.

Separate from train.py — experimental, does not touch the main pipeline.

Usage:
    uv run python src/train_swin.py --variant s
    uv run python src/train_swin.py --variant s --epochs 100 --unfreeze-after 25 --rigid-lr
    uv run python src/train_swin.py --variant b --epochs 100 --unfreeze-after 25 --rigid-lr
"""
...
from models.swin import get_swin_b, get_swin_s
...
BUILDERS = {"s": get_swin_s, "b": get_swin_b}
...
def parse_args():
    ...
    p.add_argument("--variant",             choices=["s", "b"], default="s")
...
def main():
    args      = parse_args()
    model_key = f"swin_{args.variant}"
