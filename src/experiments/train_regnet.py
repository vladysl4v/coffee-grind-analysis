"""
Train RegNet for coffee grind fineness regression.

Separate from train.py — experimental, does not touch the main pipeline.

Usage:
    uv run python src/train_regnet.py --variant 8gf
    uv run python src/train_regnet.py --variant 16gf --epochs 100 --unfreeze-after 15
    uv run python src/train_regnet.py --variant 8gf --epochs 100 --unfreeze-after 15 --flip-augment
"""
...
from models.regnet import get_regnet_y_8gf, get_regnet_y_16gf

_ROOT     = Path(__file__).parent.parent
_RUNS_DIR = _ROOT / "data" / "runs"

BUILDERS = {"8gf": get_regnet_y_8gf, "16gf": get_regnet_y_16gf}
...
def parse_args():
    ...
    p.add_argument("--variant",             choices=["8gf", "16gf"], default="8gf")
...
def main():
    args      = parse_args()
    model_key = f"regnet_y_{args.variant}"
