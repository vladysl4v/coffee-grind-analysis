"""
Train EfficientNetV2 for coffee grind fineness regression.

Separate from train.py — experimental, does not touch the main pipeline.

Usage:
    uv run python src/train_efficientnetv2.py --variant s
    uv run python src/train_efficientnetv2.py --variant s --epochs 100 --unfreeze-after 25 --rigid-lr
    uv run python src/train_efficientnetv2.py --variant m --epochs 100 --unfreeze-after 10 --lr 1e-4
"""
...
from models.efficientnetv2 import get_efficientnetv2_m, get_efficientnetv2_s

_ROOT     = Path(__file__).parent.parent
_RUNS_DIR = _ROOT / "data" / "runs"

BUILDERS = {"s": get_efficientnetv2_s, "m": get_efficientnetv2_m}

# Rigid LR schedule: list of (epoch_start, lr) — applied from that epoch onwards
RIGID_LR_SCHEDULE = [
    (1,  1e-3),
    (26, 7e-5),
    (36, 1e-5),
...
def parse_args():
    ...
    p.add_argument("--variant",             choices=["s", "m"], default="s",
                   help="EfficientNetV2 variant: s or m")
...
def main():
    args      = parse_args()
    model_key = f"efficientnetv2_{args.variant}"
